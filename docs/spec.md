...
> **Correction (2026-08-12), from building the mirror: the ~83-hour figure in conclusion 2 is wrong by an order of magnitude, because the fetch unit is the Act, not the section.**
>
> An Act's `Articles` page embeds the **full text of every section in it**, and the Acts too large for that expose one `ChapAct=FullText` link returning the whole Act as a single document. The Election Code is **963 sections in one request**. So the compiled-statute side of the corpus is a few thousand requests — single-digit hours at `Crawl-delay: 10` — not thirty thousand requests over three days. Conclusion 2's *policy* stands unchanged and is implemented literally; only its arithmetic was pessimistic, and it was pessimistic because it assumed the per-section `DocName` route (§2.1) had to be the ingest route. `DocName` remains the right *primary key*; it is not the right *fetch plan*.
>
