...
> **Correction (2026-08-12), from building the mirror: the ~83-hour figure in conclusion 2 is roughly 4–8× too pessimistic, because the fetch unit is the Act, not the section.**
>
> An Act's `Articles` page embeds the **full text of every section in it**, and the Acts too large for that expose one `ChapAct=FullText` link returning the whole Act as a single document. The Election Code is **963 sections in one request**. A full walk of the chapter/act index (68 chapters, 68 requests) enumerates **3,479 ILCS Acts** — so the compiled-statute side of the corpus costs 3,479–6,958 requests, i.e. **roughly 10–19 hours** at `Crawl-delay: 10`, against the 30,000 requests and 83 hours the estimate assumed. Conclusion 2's *policy* stands unchanged and is implemented literally; only its arithmetic was pessimistic, and it was pessimistic because it assumed the per-section `DocName` route (§2.1) had to be the ingest route. `DocName` remains the right *primary key*; it is not the right *fetch plan*.
>
> Note that 3,479 Acts is a **measured** count from the index walk, not the earlier in-flight projection of ~5,600 that a partial walk suggested. The two differ because chapter sizes are wildly uneven — chapter 70 alone holds 197 Acts while chapter 315 holds 7 — so extrapolating from the first third of the alphabet overshoots.
>
> The Public Act side is where the hours actually go, and it is bounded differently: an Act is only worth fetching if some section names it, so the count is driven by distinct terminal Source-line Acts rather than by the size of the Act corpus. Measured across the seven Acts Oracle-0 already mirrored: **4,600 sections cite 680 distinct Public Acts** at or above the corpus floor — a 6.8:1 ratio in the crawler's favour.
>
> Implementation and the queue ordering that falls out of this: `src/palimpsest/crawl.py`.
